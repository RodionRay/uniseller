...
async function loadOwnerProduct(owner:string):Promise<{product?:string;apiKey?:string}>{
  try{
    const db=database();
    const config:any=await db.prepare('SELECT * FROM records WHERE owner=? AND kind=? LIMIT 1').bind(owner,'settings').first();
    if(!config)return {};
    const data=JSON.parse(config.data);
    const apiKey=config.secret?await unseal(config.secret,owner):undefined;
    return {product:data.product,apiKey};
  }catch{
    return {};
  }
}
...
export async function POST(req:Request){
  try{
    ...
    if(bodyText.length>20_000)return reply({error:'Слишком большой запрос'},413);
    ...
    try{db=database()}catch{db=null}

    if(db){
      const guardRow:any=await db.prepare('SELECT created FROM records WHERE id=?').bind(guardId).first();
      if(!canAskAssistant(guardRow?.created,now)){
        return reply({error:'Подождите несколько секунд перед следующим вопросом.'},429);
      }
      await db.prepare(
        'INSERT INTO records(id,owner,kind,data,created) VALUES(?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET created=excluded.created',
      ).bind(guardId,owner||'public','ai_guard','{}',now.toISOString()).run();
    }

    const owned=owner?await loadOwnerProduct(owner):{};
    ...
    if(e instanceof z.ZodError){
      return reply({error:'Проверьте сообщение: '+e.issues.map((i)=>i.path.join('.')).join(', ')},400);
    }
    if(e instanceof SyntaxError)return reply({error:'Некорректный запрос'},400);
    return reply({error:'Ассистент временно недоступен. Попробуйте позже.'},503);
  }
}
